...
import { 
  ArrowLeft, Save, Plus, Trash2, Printer, Send, Calculator 
} from 'lucide-react';

import { base44 } from '@/api/base44Client';

export default function EstimateDetail() {
  // --- FIX 1: DEFINE estimateId FROM URL ---
  const { id } = useParams(); 
  const estimateId = id; // Map the URL ID to the variable name your code expects
  ...
  // Recalculates totals whenever items or tax changes
  const recalculate = (currentData) => {
    ...
    // 3. Calc Grand Total
    const tax = Number(currentData.tax_rate) || 0;
  ...
  const handleItemChange = (index, field, value) => {
    ...
    const nextState = { ...formData, items: newItems };
    setFormData(recalculate(nextState));
  };

  const handleAddItem = () => {
    const newItem = { description: '', quantity: 1, unit_cost: 0, total: 0 };
    const nextState = { ...formData, items: [...formData.items, newItem] };
    setFormData(recalculate(nextState));
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Safe Mode Update: Pass ID explicitly
  ...
  // --- RENDER ---
  if (loading) return <div className="p-8 text-center">Loading Estimate...</div>;
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {formData.items.map((item, index) => (
              <div key={index} className="grid grid-cols-12 gap-2 items-center bg-slate-50 p-2 rounded hover:bg-slate-100 transition-colors group">
            ...
            {formData.items.length === 0 && (
              <div className="text-center py-8 text-gray-400 border-2 border-dashed rounded-lg">
                No items yet. Use "Open Tools" to calculate items or "Add Item" to type manually.
              </div>
            )}
          </div>
        </CardContent>
