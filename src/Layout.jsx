...
export default function Layout({ children }) {
  ...
  // --- FIX START: EXPLICIT FILE MAPPING ---
  // This connects the URL path (left) to the exact File Name in your list (right)
  const PAGE_MAP = {
    '/': 'Dashboard',
    '/clients': 'ClientProfiles',
    '/estimates': 'JobEstimates',
    '/schedule-leads': 'ScheduleLeads',
    '/portfolio': 'Portfolio',
    '/inventory': 'Inventory',
    '/job-kits': 'JobKits',
    '/calculators': 'Calculators', // Maps to Pages/Calculators
  ...
  const navigation = [
    ...
    { name: 'Job Kits', icon: Package, path: '/job-kits' }, // This was missing in your old logic
  ...
  // SECTION 4: NOTIFICATION & BACKGROUND LOGIC (PRESERVED)
  React.useEffect(() => {
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 fixed h-full z-10">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {navigation.map((item) => {
            ...
            // --- LOGIC UPGRADE: USE THE MAP INSTEAD OF NESTED TERNARY ---
            const targetFileName = PAGE_MAP[item.path] || 'Dashboard'; 
            
            return (
              <Link key={item.name} to={createPageUrl(targetFileName)}>
                <div 
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-10 bg-white pt-20 px-4">
          <nav className="space-y-2">
            {navigation.map((item) => {
               // --- LOGIC UPGRADE FOR MOBILE TOO ---
               const targetFileName = PAGE_MAP[item.path] || 'Dashboard';
               
               return (
                <Link 
                  key={item.name} 
                  to={createPageUrl(targetFileName)}
